...
class InstagramDMScraper {
  ...
  async extractEnhancedAdReplyMessages() {
    try {
      ...
      console.log("inside fuction extractEnhancedAdReplyMessages" )
      const adReplies = await this.page.evaluate(() => {
        // --- 1. Get display name and handle from the specified class structure ---
        function getHeaderInfo() {
          let displayName = null, handle = null;
          ...
          // Instagram DM header area - try multiple possible selectors
          ...
          for (const selector of headerSelectors) {
            ...
            if (headerDiv) {
              console.log('Found header div with selector:', selector);
              break;
            }
          }
          ...
          if (headerDiv) {
            ...
            // Extract display name from h2 span
            const nameSelectors = [
              'h2 span[title]', // Try title attribute first
              'h2 span span[title]', // Nested span with title
              'h2 span',
              'h2 span span'
            ];
            ...
            for (const selector of nameSelectors) {
              ...
              if (nameEl) {
                ...
                if (displayName) {
                  console.log('Display name found with selector:', selector, 'Value:', displayName);
                  break;
                }
              }
            }
            
            // Extract handle - try multiple approaches
            
            // Method 1: From profile link href
            const profileLink = headerDiv.querySelector('a[href^="/"]');
            if (profileLink) {
              const href = profileLink.getAttribute('href');
              ...
              if (href) {
                handle = href.replace(/^\//, '').replace(/\/$/, '');
                console.log('Handle from href:', handle);
              }
            }
            
            // Method 2: From the handle span (secondary display)
            if (!handle) {
              const handleSelectors = [
                'span[class*="x1lliihq"][class*="x193iq5w"][class*="x6ikm8r"]:not([title])', // Span without title (usually handle)
                'div[class*="x1xmf6yo"] span[class*="x1lliihq"]', // Handle in the lower section
                'span[class*="xlyipyv"][class*="xuxw1ft"]' // Another possible handle selector
              ];
              
              for (const selector of handleSelectors) {
                const elements = headerDiv.querySelectorAll(selector);
                ...
                for (const el of elements) {
                  ...
                  // Handle is typically the one that starts with underscore or doesn't have spaces/emojis
                  if (text && text !== displayName && (text.includes('_') || text.includes('.') || !text.includes(' '))) {
                    handle = text;
                    console.log('Handle found from text:', handle);
                    break;
                  }
                }
                if (handle) break;
            ...
            // Method 3: Fallback - look for any text that looks like a handle
            if (!handle) {
              const allSpans = headerDiv.querySelectorAll('span');
              console.log('Fallback: checking all spans, found:', allSpans.length);
              
              for (const span of allSpans) {
                const text = span.textContent?.trim();
                if (text && text !== displayName && text.length > 0) {
                  console.log('Checking span text:', text);
                  // Look for handle patterns (contains underscore, dots, or no spaces)
                  if (text.includes('_') || text.includes('.') || (!text.includes(' ') && !text.includes('✨') && text.length > 2)) {
                    handle = text;
                    console.log('Handle found in fallback:', handle);
                    break;
                  }
                }
              }
            ...
            console.log('Header div not found, trying alternative selectors...');
            
            // Alternative approach - look for profile links anywhere
            ...
            for (const link of allProfileLinks) {
              ...
              if (href && href.length > 1) {
                handle = href.replace(/^\//, '').replace(/\/$/, '');
                console.log('Handle from alternative method:', handle);
                break;
              }
            }
          }
  ...
  //           }
  //         };
  //         console.log('🧾 Final object before saving:', message);
